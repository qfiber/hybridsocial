defmodule Hybridsocial.Repo.Migrations.CreateGroups do
  ...
  def up do
    ...
    execute(
      "CREATE TYPE group_join_policy AS ENUM ('open', 'screening', 'approval', 'invite_only')"
    )

    execute("CREATE TYPE group_member_role AS ENUM ('member', 'moderator', 'admin', 'owner')")

    execute(
      "CREATE TYPE group_member_status AS ENUM ('pending', 'approved', 'rejected', 'banned')"
    )

    execute(
      "CREATE TYPE group_application_status AS ENUM ('pending', 'approved', 'rejected', 'auto_approved')"
    )
    ...
    create table(:group_invites, primary_key: false) do
      ...
      add :invited_by, references(:identities, type: :binary_id, on_delete: :delete_all),
        null: false

      add :invited_id, references(:identities, type: :binary_id, on_delete: :delete_all),
        null: false
