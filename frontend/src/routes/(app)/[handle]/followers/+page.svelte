<script lang="ts">
  ...
  import { lookupAccount, getFollowers, follow, unfollow } from '$lib/api/accounts.js';
  ...
  async function loadFollowers(reset = false) {
    ...
    try {
      ...
      const result = await getFollowers(account.id, cursor || undefined);
...
<div class="followers-page">
  ...
  {:else if error}
    <div class="error-state">
      <p>{error}</p>
      <button type="button" class="btn btn-outline" onclick={() => loadFollowers(true)}>Retry</button>
    </div>
